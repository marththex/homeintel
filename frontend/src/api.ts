...
// Same-origin by default: the API serves the built frontend in prod, and the
// Vite dev server proxies API routes to :8000. Env var overrides both.
...
// Cached server-side thumbnail — use for grid/carousel; full-res only in the lightbox.
export function thumbUrl(path: string, w = 480): string {
  return `${BASE}/thumb?path=${encodeURIComponent(path)}&w=${w}`;
}
