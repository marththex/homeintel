...
def check_chat_resume() -> Result:
    name = "POST /chat (Jane Doe resume) → non-empty answer + resume source"
    payload = {"question": "What companies has Jane Doe worked for?"}
    resp = httpx.post(f"{BASE_URL}/chat", json=payload, timeout=REQUEST_TIMEOUT)
    body = resp.json()
    answer = body.get("answer", "")
    sources = body.get("sources", [])
    has_resume = any("jane-doe-resume" in s.get("file_name", "") for s in sources)
...
def check_chat_docker() -> Result:
    name = "POST /chat (photo service port) → non-empty answer + compose source"
    payload = {"question": "What port does the photo service run on?"}
    resp = httpx.post(f"{BASE_URL}/chat", json=payload, timeout=REQUEST_TIMEOUT)
    body = resp.json()
    answer = body.get("answer", "")
    sources = body.get("sources", [])
    has_docker = any("sample-compose" in s.get("file_name", "") for s in sources)
